...
videojs.Vimeo = videojs.MediaTechController.extend({
  init: function(player, options, ready){
    ...
    this.startMuted = player.options()['muted'] || false;
...
videojs.Vimeo.prototype.buffered = function(){ 
	if (this.vimeo && this.vimeo.getVideoBytesLoaded) {
    	var loadedBytes = this.vimeo.getVideoBytesLoaded();
    	var totalBytes = this.vimeo.getVideoBytesTotal();
    	if (!loadedBytes || !totalBytes) return 0;

    	var duration = this.vimeo.getDuration();
    	var secondsBuffered = (loadedBytes / totalBytes) * duration;
    	var secondsOffset = (this.vimeo.getVideoStartBytes() / totalBytes) * duration;
